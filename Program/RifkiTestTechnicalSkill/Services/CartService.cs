...
namespace RifkiTestTechnicalSkill.Services
{
    public class CartService : ICartService
    {
        ...
        private CalculatorSoapClient _calculatorSoapClient;


        public CartService(ApplicationDbContext db, IHttpContextAccessor httpContextAccessor,
            UserManager<IdentityUser> userManager, CalculatorSoapClient calculatorSoapClient)
        {
            _db = db;
        ...
        public async Task<int> AddItem(Guid productId, int qty)
        {
            ...
            try
            {
                ...
                {
                    cartItem.Quantity +=  await _calculatorSoapClient.AddAsync(0, qty);
                }
                else
                {
        ...
        public async Task<int> RemoveItem(Guid productId)
        {
            //using var transaction = _db.Database.BeginTransaction();
            string userId = GetUserId();
            try
            {
                ...
                if (cartItem is null)
                    ...
                    cartItem.Quantity = await _calculatorSoapClient.SubtractAsync (cartItem.Quantity, 1);
                _db.SaveChanges();
            }
            catch (Exception ex)
            {
